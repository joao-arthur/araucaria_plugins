...
mod tests {
    ...
    fn serialize_schema_err() {
        ...
        let schema_err = SchemaErr::Obj(BTreeMap::from([
            ("bool".into(), SchemaErr::Arr(vec![ValidationErr::Required, ValidationErr::Bool, bool_op])),
        ...
        assert_eq!(
            ...
            r#"{"bool":["Required","Bool",{"Operation":{"Eq":false}}],"f64":["Required","F64"],"i64":["Required","I64"],"u64":["Required","U64"]}"#
                .to_string()
        );
    }
}
