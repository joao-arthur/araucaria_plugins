...
mod tests {
    ...
    fn validate_f64_default() {
        ...
        assert_eq!(validate_f64(&v, &Value::None, &ROOT, true), Err(SchemaErr::arr([REQUIRED, F64])));
        assert_eq!(validate_f64(&v, &Value::None, &ROOT, false), Err(SchemaErr::arr([REQUIRED, F64])));
        assert_eq!(validate_f64(&v, &bool_stub(), &ROOT, false), Err(SchemaErr::arr([F64])));
    }
    ...
    fn validate_f64_optional() {
        ...
        assert_eq!(validate_f64(&v, &Value::F64(-42.5), &ROOT, false), Ok(()));
        assert_eq!(validate_f64(&v, &Value::None, &ROOT, true), Err(SchemaErr::arr([F64])));
