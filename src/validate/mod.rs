...
mod tests {
    ...
    fn validate_optional_ok() {
        ...
        assert_eq!(validate(&v_u64, &Value::None), Err(SchemaErr::validation([U64])));
        assert_eq!(validate(&v_i64, &Value::None), Err(SchemaErr::validation([I64])));
        assert_eq!(validate(&v_f64, &Value::None), Err(SchemaErr::validation([F64])));
        assert_eq!(validate(&v_usize, &Value::None), Err(SchemaErr::validation([USIZE])));
        assert_eq!(validate(&v_isize, &Value::None), Err(SchemaErr::validation([ISIZE])));
        assert_eq!(validate(&v_bool, &Value::None), Err(SchemaErr::validation([BOOL])));
        assert_eq!(validate(&v_str, &Value::None), Err(SchemaErr::validation([STR])));
        assert_eq!(validate(&v_email, &Value::None), Err(SchemaErr::validation([EMAIL])));
        assert_eq!(validate(&v_date, &Value::None), Err(SchemaErr::validation([DATE])));
        assert_eq!(validate(&v_time, &Value::None), Err(SchemaErr::validation([TIME])));
        assert_eq!(validate(&v_date_time, &Value::None), Err(SchemaErr::validation([DATE_TIME])));
        assert_eq!(validate(&v_enum, &Value::None), Err(SchemaErr::validation([ValidationErr::Enumerated(EnumValues::from(str_values.clone()))])));
    }
