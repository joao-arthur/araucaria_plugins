...
pub fn validate_date(validation: &DateValidation, value: &Value, root: &Value, enforce_optional: bool) -> Result<(), SchemaErr> {
    let mut base = vec![];
    match value {
        Value::Str(str_value) => {
...
mod tests {
    ...
    fn validate_date_operation_value() {
        ...
        assert_eq!(validate_date(&v, &Value::None, &ROOT, true), Err(SchemaErr::validation([REQUIRED, DATE, op_err.clone()])));
