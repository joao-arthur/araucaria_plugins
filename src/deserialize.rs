...
pub fn value_from_json_value(value: &serde_json::Value, validation: Option<&Validation>) -> Value {
    match value {
        ...
        serde_json::Value::Array(arr) => {
            Value::Arr(arr.into_iter().map(|item| value_from_json_value(item, None)).collect())
        }
        serde_json::Value::Object(obj) => {
            ...
            for (key, item) in obj {
                if let Some(Validation::Obj(obj_validation)) = validation {
                    ...
                    result.insert(
                        key.clone(),
                        value_from_json_value(item, obj_validation.validation.get(&fff)),
                    );
                } else {
...
mod test {
    ...
    use araucaria::validation::{
        num_f::NumFValidation, num_i::NumIValidation, num_u::NumUValidation, ObjValidation,
        Validation,
    };
    ...
    fn test_serde_json() {
        assert_eq!(
            serde_json::from_str::<serde_json::Value>("192168").unwrap().as_u64(),
            Some(192168)
        );
        assert_eq!(
            serde_json::from_str::<serde_json::Value>("192168").unwrap().as_i64(),
            Some(192168)
        );
        assert_eq!(
            serde_json::from_str::<serde_json::Value>("192168").unwrap().as_f64(),
            Some(192168.0)
        );
        ...
        assert_eq!(
            serde_json::from_str::<serde_json::Value>("-192168.5").unwrap().as_f64(),
            Some(-192168.5)
        );
    }
    ...
    fn test_value_from_json_value_primites() {
        ...
        assert_eq!(
            value_from_json_value(&serde_json::Value::Bool(false), None),
            Value::Bool(false)
        );
        assert_eq!(
                ...
                Value::from([
                    (
                        ...
                        Value::from([
                            (
                                (String::from("lvl_3")),
                                Value::from([((String::from("num")), Value::NumU(192_168)),])
                            ),
                ...
                Value::from([
                    (
                        ...
                        Value::from([
                            (
                                (String::from("lvl_3")),
                                Value::from([((String::from("num")), Value::NumI(192_168)),])
                            ),
                            ((String::from("num")), Value::NumI(-192_168)),
                        ])
                    ),
